-- SAFE IDP Extension for Existing Supabase Project
-- This script safely adds IDP features without impacting existing data
...
-- 2. Safely update profiles table to include organization fields
DO $$ 
BEGIN
...
-- 4. Safely extend documents table for IDP features
DO $$ 
BEGIN
    -- Add organization_id column if it doesn't exist
...
-- 11. Update existing documents policies (safely)
DO $$ 
BEGIN
    ...
    IF NOT EXISTS (SELECT FROM pg_policies WHERE tablename = 'documents' AND policyname = 'Users can view organization documents') THEN
        ...
        RAISE NOTICE 'Created new documents policy';
    END IF;
END $$;
...
DO $$ 
BEGIN
    RAISE NOTICE '✅ IDP Extension completed successfully!';
    ...
    RAISE NOTICE '🛡️ Enabled RLS policies for security';
