    ...
    def __init__(self):
        ...
        try:
            ...
            self.layout_model = LayoutLMv3ForTokenClassification.from_pretrained(
                layout_model_name,
                num_labels=11  # Match our label2id count for proper field classification
            ).to(self.device)
    ...
    def process_image(self, image: Image.Image) -> Dict[str, Any]:
        ...
        try:
            ...
            if getattr(self, 'use_layoutlm', False) and self.layout_processor and self.layout_model:
                ...
                try:
                    layoutlm_fields = self._extract_fields_layoutlm_universal(image, extracted_text, bounding_boxes)
                    extracted_fields.update(layoutlm_fields)
                except Exception as e:
                    logger.warning(f"LayoutLM extraction failed: {str(e)}")
            
            # Always use pattern-based extraction as primary method
            logger.info("Using pattern-based field extraction")
            pattern_fields = self._extract_fields_pattern_based(extracted_text)
            extracted_fields.update(pattern_fields)
            
            # If pattern-based extraction failed, try alternative approaches
