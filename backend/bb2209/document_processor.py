...
class DocumentProcessor:
    ...
    def process_image(self, image: Image.Image) -> Dict[str, Any]:
        ...
        try:
            ...
            # Donut fallback: if overall OCR confidence is low and Donut is enabled, run Donut and merge/replace
            if self.use_donut and (not bounding_boxes or (sum(b['confidence'] for b in bounding_boxes) / max(1, len(bounding_boxes))) < self.donut_threshold):
                try:
                    logger.info("Applying Donut fallback due to low OCR confidence")
