...
class DonutDocumentProcessor:
    def __init__(self):
        ...
        try:
            ...
            # Configure model for generation with proper token IDs
            ...
            # Ensure decoder configuration is proper
            if hasattr(self.model.config, 'decoder'):
                self.model.config.decoder.decoder_start_token_id = self.processor.tokenizer.bos_token_id
                self.model.config.decoder.pad_token_id = self.processor.tokenizer.pad_token_id
                self.model.config.decoder.eos_token_id = self.processor.tokenizer.eos_token_id
    ...
    def process_document(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Prepare task prompt for structured data extraction
            task_prompt = "<s_docvqa>"
            
            try:
                # Use the processor's built-in task prompt handling
                ...
                # Ensure decoder input is within valid range
                max_decoder_length = self.model.config.decoder.max_position_embeddings
                if decoder_input_ids.max().item() >= max_decoder_length:
                    logger.warning(f"Decoder input contains tokens >= {max_decoder_length}, truncating...")
                    decoder_input_ids = torch.clamp(decoder_input_ids, 0, max_decoder_length - 1)
                
                logger.info(f"Decoder input shape: {decoder_input_ids.shape}")
                logger.info(f"Decoder input max token: {decoder_input_ids.max().item()}")
                logger.info(f"Decoder max length: {max_decoder_length}")
                ...
                try:
                    outputs = self.model.generate(
                        ...
                        no_repeat_ngram_size=0,  # Disable to avoid issues
                ...
                logger.info(f"Raw model output: '{extracted_text}'")
                
                # Check if the model output is meaningful
                if extracted_text and len(extracted_text) > 3 and not self._is_repetitive_output(extracted_text):
                    # Post-process the extracted text to get structured data
                    structured_data = self._parse_model_output(extracted_text, image)
                    logger.info(f"Successfully extracted structured data: {len(structured_data)} fields")
                else:
                    logger.warning("Model output is not meaningful, using OCR fallback")
                    # Force OCR fallback for better extraction
                    structured_data = self._extract_using_ocr_fallback(image)
                    extracted_text = " ".join(structured_data.values()) if structured_data else ""
                    
            except Exception as e:
                logger.error(f"Error during text generation: {str(e)}", exc_info=True)
                logger.error(f"Exception type: {type(e)}")
                logger.error(f"Pixel values shape: {pixel_values.shape if 'pixel_values' in locals() else 'Not available'}")
                logger.error(f"Device: {self.device}")
                
                # Try fallback approach without decoder input
                try:
                    logger.info("Attempting fallback generation without decoder input...")
                    outputs = self.model.generate(
                        pixel_values,
                        max_length=256,
                        min_length=10,
                    ...
                    # Decode the sequence
                    extracted_text = self.processor.tokenizer.decode(outputs[0], skip_special_tokens=True)
                    
                    if extracted_text and len(extracted_text) > 3:
                        logger.info(f"Fallback extraction successful. Length: {len(extracted_text)}")
                        # Parse the fallback output
                        structured_data = self._parse_model_output(extracted_text, image)
                    else:
                        logger.warning("Fallback extraction produced no meaningful text")
                        # Use OCR fallback
                        structured_data = self._extract_using_ocr_fallback(image)
                        extracted_text = " ".join(structured_data.values()) if structured_data else ""
            ...
            # Use structured data if available, otherwise process extracted text
            if 'structured_data' in locals() and structured_data:
                fields = structured_data
                doc_type = self._detect_document_type(" ".join(structured_data.values()))
                logger.info(f"Using structured data with {len(fields)} fields")
            else:
                fields = {}
                doc_type = "unknown"
            
            # Process the extracted text only if we don't have structured data
            if not fields:
                try:
                    # Determine document type if not already determined
                    if doc_type == "unknown":
    ...
    def _parse_model_output(self, text: str, image: Image.Image) -> Dict[str, str]:
        """Parse the raw model output to extract structured data."""
        fields = {}
        try:
            logger.info(f"Parsing model output: '{text}'")
            
            # If the model output contains field names instead of values, use OCR fallback
            if any(field_name in text.lower() for field_name in ['invoice_number', 'date', 'amount', 'vendor', 'customer']):
                logger.warning("Model output contains field names, using OCR fallback")
                return self._extract_using_ocr_fallback(image)
            ...
            if not fields:
                fields = self._extract_using_patterns(text)
            
            logger.info(f"Parsed {len(fields)} fields from model output")
            return fields
    ...
    def _extract_using_ocr_fallback(self, image: Image.Image) -> Dict[str, str]:
        """Fallback extraction using OCR and pattern matching."""
        fields = {}
        try:
            logger.info("Using OCR fallback extraction")
            
            # Convert image to text using OCR with multiple PSM modes for better results
            import pytesseract
            
            # Try different PSM modes for better text extraction
            psm_modes = ['6', '3', '1']  # Different page segmentation modes
            best_text = ""
            ...
            logger.info(f"Best OCR extracted text length: {len(best_text)}")
            logger.info(f"OCR text preview: {best_text[:200]}...")
            
            if best_text.strip():
                # Extract fields using patterns
                fields = self._extract_using_patterns(best_text)
                logger.info(f"OCR fallback extracted {len(fields)} fields")
                
                # If no fields found, try more aggressive patterns
                if not fields:
                    logger.info("Trying more aggressive pattern matching...")
                    fields = self._extract_using_aggressive_patterns(best_text)
    ...
    def _extract_using_aggressive_patterns(self, text: str) -> Dict[str, str]:
        """More aggressive pattern matching for difficult documents."""
        fields = {}
        try:
            # Look for any alphanumeric patterns that could be invoice numbers
            invoice_matches = re.findall(r'\b[A-Z]{2,}[-\s]?\d{3,}\b', text)
            if invoice_matches:
                fields['invoice_number'] = invoice_matches[0]
            ...
            # Look for any amount-like patterns
            amount_matches = re.findall(r'\$\s*\d+(?:,\d{3})*(?:\.\d{2})?', text)
            if amount_matches:
                fields['total_amount'] = amount_matches[0].replace('$', '').strip()
            
            # Look for company names (capitalized words)
            company_matches = re.findall(r'\b[A-Z][A-Za-z]+\s+(?:Inc|Corp|LLC|Ltd|Company|Co)\b', text)
            if company_matches:
                fields['vendor_name'] = company_matches[0]
            
            logger.info(f"Aggressive pattern extraction found {len(fields)} fields")
            return fields
            
        except Exception as e:
            logger.error(f"Error in aggressive pattern extraction: {str(e)}")
            return {}

    def _extract_using_patterns(self, text: str) -> Dict[str, str]:
        ...
        try:
            ...
            company_patterns = [
                r'(?:from|vendor|company)[\s:]*([A-Za-z][A-Za-z\s&.,]+)',
            ...
            for pattern in company_patterns:
                match = re.search(pattern, text, re.IGNORECASE)
                if match:
                    fields['vendor_name'] = match.group(1).strip()[:50]  # Limit length
                    break
            
            logger.info(f"Pattern extraction found {len(fields)} fields")
            return fields
    ...
    def _is_repetitive_output(self, text: str) -> bool:
        """Check if the model output is repetitive or meaningless."""
        if not text:
            return True
        
        # Check for repetitive characters
        if len(set(text)) <= 2:  # Only 2 or fewer unique characters
            return True
        ...
        meaningless_patterns = ['IIIIIIIIIIIIII', 'AAAAAAAAAAAAAA', '00000000000000', '11111111111111']
        if text in meaningless_patterns:
            return True
